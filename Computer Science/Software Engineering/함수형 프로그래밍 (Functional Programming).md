...
- 함수 자체가 독립적이며 side-effect가 없기 때문에 Thread에 안전성을 보장받을 수 있다. (순수함수는 메모리, I/O의 관점에서 side effect가 없기 때문에 안전성을 보장받는 것이다.)
- Thread에 안전성을 보장받아 병렬처리를 동기화없이 진행할 수 있다.

### 🦁 1급객체 (first class object)란

1급 객체는 다음과 같은 조건을 만족하는 것들이 1급객체가 될 수 있다.
...
함수형 프로그래밍에서 함수는 1급객체로 취급받기 때문에 위 예제에서 본  ```process(10, print(num))``` 처럼 함수를 파라미터로 넘기는 등의 작업이 가능한 것이다.

### 🦁 참조 투명성 (Referential Transparency)

```
- 동일한 인자에 대해 항상 동일한 결과를 받아야 한다.
