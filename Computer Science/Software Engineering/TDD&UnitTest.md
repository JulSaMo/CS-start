...
#### 아니 근데,,, 상식적으로 테스트를 할려면 뭘 만들어야 테스트 할 수 있는 거아님?? 작업과 테스트를 동시에 진행한다는게 뭘말일까?? 그래서 제가 한번 찾아봤습니다.. 
...
#### 개인적으로 TDD 방식은 퀴즈 같은거입니다.
내가 이 테스트를 통과하도록 코드를 짜고 싶으면 어떻게 해야하지? 를 먼저 설정해놓고 그에 맞게 돌아가서 코드를 고치는 것입니다.
예를 들어서 위의 테스트를 통과하려면 stirng 함수의 반환값을 "0.00"에 맞춰야합니다. 그럼 MoneyFormatter 파일에 들어가서 그 부분만 고치는 것이죠 
