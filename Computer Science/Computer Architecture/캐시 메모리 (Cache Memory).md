...
- Conflict Miss: 캐시 메모리에 A와 B 데이터를 저장해야 하는데, A와 B가 같은 메모리 주소에 할당되어 있어서 나는 미스 (direct mapped cache 에서 많이 발생)
- Capacity Miss: 캐시 메모리의 공간이 부족해서 나는 미스. (Conflict 는 주소 할당 문제, Capacity는 공간 문제라고 볼 수 있음)
